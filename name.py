...
# -----------------------------------------------------------------------------
# 3. 2.0% 기준 0.3%p 단위 구간 설정 (Custom Bins)
# -----------------------------------------------------------------------------
bins = [-float('inf'), 1.4, 1.7, 2.0, 2.3, 2.6, float('inf')]
labels = [
    '1.4% 미만',
    '1.4% ~ 1.7% 미만',
    '1.7% ~ 2.0% 미만',
...
# -----------------------------------------------------------------------------
# 범주별 색상 매핑 (비율이 낮을수록 옅은 연두/노랑계열, 높아질수록 진한 파랑/보라계열)
color_discrete_map = {
    '1.4% 미만': '#f7fcf5',
    '1.4% ~ 1.7% 미만': '#e0f3f8',
    '1.7% ~ 2.0% 미만': '#9ecae1',
...
fig = px.choropleth(
    ...
    color='rate_group',               # 0.3%p 단위 범주 열 지정
    color_discrete_map=color_discrete_map,
    category_orders={'rate_group': labels}, # 범례 순서 고정
...
fig.update_layout(
    ...
    legend_title_text="비율 구간 (2.0% 기준 0.3%p 단위)",
    height=650
)
